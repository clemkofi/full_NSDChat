...
import com.finalyear.networkservicediscovery.utils.FileOperator;
...
public class ChatUIScreen extends javax.swing.JFrame {
    ...
    String selectedUser;
    String homePath = "G:\\project files\\servcie discovery desktop\\NSDChat_today\\message_store\\";

    public ChatUIScreen(boolean isServer, String selectedUser) {
        ...
        try {
            String storedText = FileOperator.getInstance().readFile(homePath,selectedUser, StandardCharsets.UTF_8);
            taClientOutput.setText(storedText);
            taClientOutput.setCaretPosition(taClientOutput.getDocument().getLength());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public boolean showChatMessage(String s, boolean isReceived) {
        ...
        if(isReceived){
            ...
            textToUse = s.replace(s.substring(0,s.indexOf('\t')),selectedUser+":");
        }else{
            textToUse = s.replace(s.substring(0,s.indexOf('\t')),"Me:");
        }

        if(!textToUse.contains("##")){
            taClientOutput.setText(taClientOutput.getText() + "\n" + textToUse);
            FileOperator.getInstance().writeFile(homePath+selectedUser,"\n"+textToUse);
        }
        taClientOutput.setCaretPosition(taClientOutput.getDocument().getLength());
        tfClientInput.setText("");
    ...
    public void main(String identity, InetAddress ip, int port) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ...
                new ChatUIScreen(isServer, selectedUser).setVisible(true);
                initialized = true;
            }
        });
